...
public class NovoRel extends SimpleReport {
	...
	protected void detail(Detail d) {
		...
		// d.addChart().addChartLine().addFields("teste", "eixox",
		// "eixoY").addDataSet(list).build();;
		// d.addChart().addChartLine().addFields("teste", "eixox",
		// "eixoY").addDataSet(1, "serie 1", "su3").addDataSet(2, "serie 1",
		// "su4")
		// .addDataSet(3, "serie 1", "su5").addDataSet(5, "serie 1",
		// "su1").addSize(500, 200).build();
		// d.addChart().addChartPie().addTitle("pizza").addDataSet("aaa",
		// 50).addDataSet("bb", 100).addDataSet("ddd", 150).build();
