...
public class MainReportListAppComplex {
	...
	public static void main(String[] args) throws IOException {
		...
		report.generateSplitPdfZip("target/");
		report.generate("target/");
		
		ByteArrayInputStream byteArrayzip = report.generateSplitPdfZip();
		...
		ByteArrayInputStream b = report2.generateSplitPdfZip();
		FileUtils.writeByteArrayToFile(new File("target/relatoriocompletoLinked.zip"), IOUtils.toByteArray(b),true);
		
		report2.generateSplitPdfZip("target/");
		report2.generate("target/");
